...
export default function ImageList({updateSelectedImageIndex}) {
  ...
  return (
    <>
        ...
        {isShowingAllImages && <div className='grid grid-cols-2 landscape:grid-cols-3 sm:grid-cols-3 w-full gap-y-6 sm:gap-2 place-items-center mb-12'>

            {/* IMAGES MAP */}
            {edieImages.map((img, index) => {

                {/* IMAGE HASHLINK */}
                return <Link href='/media/images/#image-viewer' key={index} onClick={() => handleOnClick(index)} className='h-[160px] w-[160px] md:h-[200px] md:w-[200px] relative'>
